...
compare <- data.frame(Method = c('Logistic Regression (cutoff = 0.5)', 'Random Forest (mtry = 5)', 'Adaptive boosting'), Accuracy = NA, Sensitivity = NA, Specificity = NA, Precision = NA, Recall = NA, FScore = NA, 'ROC AUC' = NA)

compare$Accuracy <- c(test_con_mat_50$overall["Accuracy"], test1_cm_rf$overall['Accuracy'],test1_cm_ab$overall['Accuracy'])

compare$Sensitivity <- c(test_con_mat_50$byClass["Sensitivity"], test1_cm_rf$byClass['Sensitivity'],test1_cm_ab$byClass['Sensitivity'])

compare$Specificity <- c(test_con_mat_50$byClass["Specificity"], test1_cm_rf$byClass['Specificity'],test1_cm_ab$byClass['Specificity'])

compare$Precision <- c(test_con_mat_50$byClass["Precision"], test1_cm_rf$byClass['Precision'],test1_cm_ab$byClass['Precision'])

compare$Recall <- c(test_con_mat_50$byClass["Recall"], test1_cm_rf$byClass['Recall'],test1_cm_ab$byClass['Recall'])

compare$FScore <- c(test_con_mat_50$byClass["F1"], test1_cm_rf$byClass['F1'],test1_cm_ab$byClass['F1'])

compare$ROC.AUC <- c(0, auc_test1_rf@y.values, auc_test1_ab@y.values)
