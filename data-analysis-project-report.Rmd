...
<span style="color:Blue"> With cutoff of 0.5 probability , we see `r confusion_matrix[2,1]` false positives and `r confusion_matrix[1,2]` false positives among 16000 observations which is less than 10%. We can try to identify cut off where this number goes down.</span>
...
In general, we can apply the below criterions to evaluate and select the model. Let's analyze the model which we worked on in this study.
