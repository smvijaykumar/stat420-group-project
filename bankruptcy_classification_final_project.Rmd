...
```{r echo=TRUE, message=FALSE,warning=FALSE,fig.width=10 }
bankruptcy<-fread('data/bankruptcy.csv')
...
X<-scale(subset(train, select=-c(DLRSN,FYEAR)))
X<-as.matrix(X)
...
```{r echo=TRUE, message=FALSE,warning=FALSE,fig.width=10 }
full.rpart<-rpart(data = subset(train,select = -c(FYEAR)),DLRSN~.,method = 'class')
plot(full.rpart)
text(full.rpart)
plotcp(full.rpart)
printcp(full.rpart)
...
**Random Forest**
```{r echo=TRUE, message=FALSE,warning=FALSE,fig.width=10 }
full.randomForest<-randomForest(data=subset(train,select = -c(FYEAR)),DLRSN~.,ntree=1000)
